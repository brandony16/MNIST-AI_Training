  ...
  # Pads image
    ...
    def forwardPass(self, inputs):
      ...
      for b in range(batch_size):
          if input_depth == 1:
              output[b] = ConvLayer._convolve_2d(inputs_padded[b, 0], self.filters, self.stride)
          else:
              output[b] = ConvLayer._convolve_3d(inputs_padded[b], self.filters, self.stride)
      
      return output

    @staticmethod
    def _backprop_2d(d_output, input_padded, filters, filter_size, stride):
    ...
    @staticmethod
    def _backprop_3d(d_output, input_padded, filters, filter_size, stride):
        num_filters, output_height, output_width = d_output.shape
        d_filters = np.zeros_like(filters, dtype=np.float64)
        d_input_padded = np.zeros_like(input_padded, dtype=np.float64)
    ...
    def backprop(self, d_output, learning_rate):
        ...
        for b in range(batch_size):
            if input_depth == 1:
                d_f, d_input_padded = self._backprop_2d(d_output[b], input_padded[b, 0], self.filters, self.filter_size, self.stride)
                d_input[b, 0] = d_input_padded[:input_height, :input_width]  # Assuming padding='same'
            else:
                d_f, d_input_padded = self._backprop_3d(d_output[b], input_padded[b], self.filters, self.filter_size, self.stride)
...
# Example usage
conv_layer = ConvLayer(num_filters=6, filter_size=3, stride=1, padding='valid')

# Initial input without channel dimension
input_tensor = np.random.rand(32, 6, 28, 28)
